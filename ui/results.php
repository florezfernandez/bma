...
for($i=0; $i<count($positions); $i++){
	...
	$position->select();
	$positionData[$i]=$position->getPosition();
}
					...
					<div class='alert alert-success' role='alert'>
						...
						<?php
							...
							for($i=0; $i<count($positions); $i++){
								...
								$positionNumbers.=$position -> getPosition().". ";
							}
						?> 
						<br>
				...
				<?php 						
						for($i=0; $i<count($patientsFilesName); $i++){
							...
							for($j=1; $j<count($patientData); $j++){
								...
								list($changes, $changesCount, $equChanges, $equChangesConf)=calculateTripletsChanges($gene->getSequence(), strtoupper($patientDataArray[1]), $positionData);
...
//								echo $secuenceChanges[$j][0]."<br>";
//								echo $secuenceChanges[$j][1]."<br>";
//								echo $secuenceChanges[$j][2]."<br>";
...
<?php
$fileStream=graphStream($gene->getName()." Con-1(1b)", $secuenceChanges);
setFileContent($graphFilePath, $fileStream);
createPDF($reportFilePath, $gene->getName()." Con-1(1b)", $patientsFilesName, $positionNumbers, $reportSummary, $reportNucleotide, $reportAminoacid);
